# Collibra Backend Interview assessment  
...
Implement a simple string protocol communicating over sockets. Server is open on port 50000 and it is listening for upcoming connections (Clients). 
After connection has been established Server and Client exchange information's where Client provides custom typed input and Server is responding with appropriate message. 
Following commands are available for Client to type after providing his name:
...
From the CMD find directory where project is extracted and navigate to Client.jar and Server.jar and execute: java -jar (path) Server/Client.jar or java -jar Server/Client.jar if path is aldready 
navigated.

### Note
```
- Server needs to be run first and then as many Clients as we want
```

## Running the tests

Test are not fully inplemented :/.
